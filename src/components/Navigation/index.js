...
class Navigation extends Component {
  ...
  componentDidMount() {
    if (!this.props.user || this.props.user.role !== "Shipper") {
      return;
    }
  ...
  renderLoader = () => {
    if (!this.props.user || !this.props.user.loader) {
      return null;
    }
  ...
  render() {
    let badge = null; // Initialize badge as null

    // Ensure user and user.role are defined
    if (this.props.user && this.props.user.role === "Shipper") {
      // Ensure requests is defined and is an array
      const requestCount = Array.isArray(this.state.requests) ? this.state.requests.length : 0;

      badge = (
        <Badge badgeContent={requestCount} badgeStyle={styles.badge} onClick={this.handleDialog}>
